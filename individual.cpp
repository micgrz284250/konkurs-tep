...
vector<individual> individual::cross(individual &ind) {
    ...
    if (real_dist(rng) <= CROSS_PROB) {
        ...
        vector genotype_1_b(get_genotype_ref().begin() + split_index, get_genotype_ref().end());

        vector genotype_2_a(ind.get_genotype_ref().begin(), ind.get_genotype_ref().begin() + split_index);
        vector genotype_2_b(ind.get_genotype_ref().begin() + split_index, ind.get_genotype_ref().end());
